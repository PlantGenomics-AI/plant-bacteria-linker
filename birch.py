...
# Avalia Birch para um dado número de clusters
def avaliar_birch(n_clusters):
    try:
...
def main():
    ...
    resultados = []
    if os.path.exists('resultados_birch.pkl'):
    ...
    with ProcessPoolExecutor(max_workers=6, initializer=init_globals, initargs=(proj, y)) as exe:
        for idx, k in enumerate(ks, start=1):
            res = exe.submit(avaliar_birch, k).result()
            ...
            # Salva a cada batch
            if idx % batch == 0:
                with open('resultados_birch.pkl','wb') as f:
    ...
    with open('resultados_birch.pkl','wb') as f:
        pickle.dump(resultados, f)
    print("Resultados finais salvos em 'resultados_birch.pkl'")

    df = pd.DataFrame(resultados)
    print("\n✅ Resultados Birch encontrados:")
    print(df)
