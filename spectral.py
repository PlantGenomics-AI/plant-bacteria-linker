...
# Função para avaliar o SpectralClustering com as métricas fornecidas
def avaliar_spectral(n_clusters):
    try:
        pid = os.getpid()
...
def main():
    ...
    with ProcessPoolExecutor(max_workers=6, initializer=init_globals, initargs=(proj, y)) as executor:
        ...
        for resultado in executor.map(avaliar_spectral, intervalos):
            count += 1
            if resultado:
